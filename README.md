...
### Описание
Библиотека представляет собой подключаемый модуль, способный хранить в себе коллекцию и сохранять ее при каждом изменении в localStorage браузера. Предусмотрена возможность "подписки" на изменения: необходимая функция будет вызываться каждый раз, когда данные изменились. Это позволяет строить интерфейсы с динамически обновляемым контентом. Библиотека поддерживаем импорт/экспорт в формате JSON.  
Как работать с библиотекой описано [здесь](https://github.com/oktava6/schedule-api/blob/master/lib-api.md).
